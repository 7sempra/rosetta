...
Parser.prototype = {
  ...
  expect: function(type) {
    if (this.t.type !== type) {
      this.error('Unexpected token "' + Token.typeToCharOrDesc(this.t.type) +
  ...
  error: function(msg) {
    throw new errors.CompilerError('ParseError', msg, this.t);
  },

  compilerError: function(msg) {
    throw new errors.CompilerError(msg);
  },
    ...
    while(this.t) {
      if (t = this.accept(Token.INDENT)) {
        ...
        if (ns.indentLen == null) {
          ...
          error('Inconsistent indentation. Expected ' + ns.indentLen +
            ' whitespace chars but found ' + indentLen + '.');
        }
    ...
    if (this.accept(Token.LPAREN)) {
      ...
      switch(t.type) {
        ...
        case Token.IDENT:
          ...
          break;
        default:
          error('Unexpected token: "' + Token.typeToCharOrDesc(t) + '"');
          break;
      }
    }
