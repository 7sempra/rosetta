...
var Resolver = module.exports = classdef({
  ...
  _computeVarValue: function(scope, refNode) {
    ...
    if (refNode.type === 'property') {
      ...
      if (!refDef) {
        error(refNode.left, 'Cannot find variable definition for ' +
            pathToStr(refNode));
      }
      ...
      if (!refDef) {
        error(refNode, 'Cannot find variable definition for "$' + token.val +
            '"');
      }
    } else {
      compilerError();
    ...
    /*if (!refDef) {
      error(node.token, 'No definition found for "' + node.token.val +
        '"');
    }*/
  ...
  _resolveAbsolutePath: function(propNode) {
    ...
    while (n) {
      if (n.type === 'property') {
        ...
        //console.log('VARS:', n.token.val, ns.vars);
        //console.log('BLARGH:', ns.vars[n.token.val]);
  ...
  for (var a = 0; a < path.length; a++) {
    ...
    if (!ns.namespaces[nsName]) {
      ns.namespaces[nsName] = new Scope(
        nsName,
        ns,
        ns.path.concat(path.slice(0, a), nsName)
      );
    }
...
function computeUnOp(op, right) {
  ...
  //console.log('RIGHT:', right.val, typeof right.val);
...
function _parseColorExpr(expr) {
  ...
  var g = _parseColorCell(expr.r);
  var b = _parseColorCell(expr.r);
  var a = null;
  if (expr.a) {
