...
exports.compile = function(sources, options, callback) {
  ...
  _.defaults(options, {
    ...
    cssFormat: 'stylus',
    jsOut: 'vars.js',
    cssOut: 'vars.' + css.getFileExtension(options.cssFormat || 'stylus') || 'css'
  });
  ...
  var parseComplete = _.after(sources.length, function() {
    console.log('All parses complete!');
    var resolvedScope;
  ...
  _.each(sources, function(path) {
    parseFile(path.resolve(path), function(ast) {
      resolver.addAst(ast);
      parseComplete();
    });
  });
...
function parseFile(path, callback) {
  fs.readFile(path, 'utf8', function(err, data) {
    ...
    try {
      ...
      console.log(debug.printTokenStream(tokens));
      var ast = parser.parse(tokens);
      console.log(debug.printAst(ast));
      callback(ast);
